...
int main() { 
	...
	int nconf = 1;
	for(int iblk=1; iblk <= nblk; ++iblk) 			//Simulation. Cycle over blocks
	{
    ...
    /*  if(istep%10 == 0){ //Saving the frames for using Ovito
    	...
    	for (int j=i+1; j < npart; ++j) {
    		...
    		if(dr < rcut) {
     			vij = 1.0/pow(dr,12) - 1.0/pow(dr,6);
       			wij = 1.0/pow(dr,12) - 0.5/pow(dr,6); // contribution to energy and virial
       			v += vij;
       			w += wij;
     		}
    	}	          
