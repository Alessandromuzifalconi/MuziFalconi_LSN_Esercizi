...
int main(){ 
	...
	int nconf = 1;
  	...
  	for(int iblk=1; iblk <= nblk; ++iblk) { 			//Simulation. Cycle over block 
		Reset(iblk);   								//Reset block averages
		for(int istep = 1; istep <= nstep; ++istep){
			Move();           			//Move particles with Verlet algorithm
			if (istep == nstep-1 && iblk == nblk) {  	//At the step before the last write the old final configuration 
				WriteOldFinal();
		    }
		       	Measure();
		  		Accumulate(); 							//Update block averages  			//Properties measurement
...
void Input(void){ 							//Prepare all stuff for the simulation
	...
	iv = 0; 							//Potential energy
  	...
  	igofr= 2; 	 						//Radial distribution function 
  	nbins = 100;						//Measurement of g(r)
  	n_props = n_props + nbins;			
  	bin_size = (box/2.0)/(double)nbins; //Divide half the size of the box for the number of bins
	...
	if (restart == 0) { 			//Read initial configuration
		...
		T = sumv2 / 3.0;									//Compute the temperature for our velocity
		fs = temp / T;										//Rescaling factor for the velocities
		for (int i = 0; i < npart; ++i) {
			vx[i] *= fs;
			vy[i] *= fs;
			vz[i] *= fs;
...
void Measure(){ 						//Properties measurement
	...
	for (int k = igofr; k < igofr+nbins; ++k) {
  	...
  	walker[iv] = 4.0 * v; 			//Array with variables for writing in blocks
	//Kinetic energy
	for (int i=0; i<npart; ++i) { 
...
void Averages(int iblk) {//Print results for current block
	...
	//g(r)
 	Gofr << iblk << "\t"; 		//Block index
    for (int k = 0;k < nbins;k++) {
        double DV = 4.0/3.0*M_PI*(pow ((k + 1)*bin_size, 3) - pow(k*bin_size, 3));
        blk_av[igofr + k] /= (rho*npart*DV);
        Gofr << blk_av[igofr + k]/blk_norm << "\t"; //100 columns with the histogram 
    }
    Gofr << endl;
    ...
    if (iblk == nblk) { 														//If we are in the last block
        for (int k = 0;k < nbins;k++) {											//Cycle over bins
            err_gdir = Error(glob_av[igofr+k], glob_av2[igofr+k], iblk);
            Gave << (k+1)*bin_size << "\t" << glob_av[igofr + k]/(double)iblk << "\t" << err_gdir << endl;   //r << g(r) << err
        }
