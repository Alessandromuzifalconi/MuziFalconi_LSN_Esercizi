...
/*
Realizza il path integral Monte Carlo.
*/
#include <iostream>
#include <fstream>
#include <cmath>
#include <TRandom3.h> //generatore di numeri casuali
#include "constants.h"
#include "functions.h"
...
int main()
{
        readInput();  //legge input
	initialize(); //inizializza il sistema, usando un polimero ad anello o lineare in base alla temperatura
...
opportunely initialized by the initialize() function. */
	...
	{
		if(PIGS)   // only a PIGS polymer has a start and an end. Se sono a T=0 PIGS=1, sono nel GS a T=0; Path Integral Ground State = PIGS 
		{
			brownianMotion(LEFT); //Quando la catena è aperta mi muovo con un moto browniano in cui distruggo e ricostruisco la coda a dx o sx del polimero  
			brownianMotion(RIGHT);
		}
		//stiamo facenodo importance sampling, campioniamo la parte cinetica in modo esatto  
		translation(); //traslazione rigida del polimero sia a T=0 che a T finita
		
		for(int j=0;j<brownianBridgeAttempts;j++) //Ponte browniano, tagliamo un pezzo di catena e lo ricostruiamo con una nuova configurazione che verrà accettata o meno con un Metropolis che ha dentro solo il potenziale, no parte cinetica
			brownianBridge();
	}
	
	for(int b=0;b<blocks;b++) //ciclo sui blocchi
	{
		for(int i=0;i<MCSTEPS;i++) //ciclo sui passi dentro ai blocchi
		{
			if(PIGS)
			{
				brownianMotion(LEFT);
			...
			upgradeAverages(); //Upgrade of the block averages	
		}
...
void initialize()
{
	...
	if(temperature==0) //See if we are in the ground state or not
		PIGS=1;
	else
		PIGS=0;
	
	if(PIGS)
		dtau = imaginaryTimePropagation/(timeslices-1); //timeslices = M in the lecture slides 
	else
		dtau = hbar/(boltzmann*temperature*timeslices); //dtau is deltatau = beta/M
...
{
	//double k_elastic = 1;					//Harmonic potential
	//return k_elastic*val*val/2.0;
	return val*val*val*val - 2.5*val*val;	//Our external potential
}
...
double variationalWaveFunction(double v) {
	//return 1.0;					//Constant
	//return exp(-0.5*v*v); 		//Single gaussian: GS of harmonic oscillator
	
	double mu = 0.79;				//My parameters
	double sig = sqrt(0.42);
	return exp(-pow(v - mu, 2) / (2 *pow(sig, 2))) + exp(-pow(v + mu, 2)/(2*pow(sig, 2))); //My wavefunction
}
...
void translation() //transaltes the polymer
{
	totalTranslations++;
