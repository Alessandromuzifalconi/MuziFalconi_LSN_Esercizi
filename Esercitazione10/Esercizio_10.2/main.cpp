...
int main (int argc, char *argv[]){
	...
	RandomInitializer(0);					//Initialize random number generator. The cities must be the same for all the continents
	...
	RandomInitializer(rank);				//Initialize the pseudo random numbers generator with different seeds for different nodes	
	Individual I[P];						//Population


	//Create the first generation
    cout << "Create the first generation of a population of " << P << " individuals in continent " << rank	<< endl << "----------------------------------------------------------------" << endl;
    
    Population_generator(I);				//Create the population
	Order_Individuals(I,order);				//Order individuals from best to worst
	
	
	//Each continent saves on its own file	
	if (rank == 0) {
   		HoF.open("Hall_of_Fame0.dat");
   		cout << "Migrations: " << Nmigr << ". One migration every: " << Mig << " generations." << endl;
 	}
 	if (rank == 1)
   		HoF.open("Hall_of_Fame1.dat");
 	if (rank == 2) 
   		HoF.open("Hall_of_Fame2.dat");
 	if (rank == 3)
   		HoF.open("Hall_of_Fame3.dat");

   	for (int g = 1; g <= Generations; g++) {	//Cycle over generations 
		if (g%Mig == 0)	{						//Make a migration
			for (int i = 0; i < N; i++) {		//Prepare individuals to be sent
				sent[i] = I[0].GetGene(i);
			}	
			
			if (rank == 0) {					//Initialize the migration
				cout << "Migration " << n << " starting: ";
				n += 1;
				r = Determine_Mig ();
			}
			
			MPI_Bcast(&r, 1, MPI_INTEGER, 0, MPI_COMM_WORLD);	//Tell everybody how the migration is going to be
			
			if (r == 1) {    									
            	Migration_A(sent,received,rank);
            }
            else if (r == 2) { 
            	Migration_B(sent,received,rank);  
            }
            else { 				
               	Migration_C(sent,received,rank);  
            }
            I[0].SetSequence(received);	
		...
		if (rank == 0)
			HoF << g << "\t" << L(I[0].GetSequence()) << "\t" << mean(P/2,lenghts) << "\t" << mean(P,lenghts) << endl; //Write on file what has happened in this generation
		if (rank == 1)
			HoF << g << "\t" << L(I[0].GetSequence()) << "\t" << mean(P/2,lenghts) << "\t" << mean(P,lenghts) << endl; //Write on file what has happened in this generation
		if (rank == 2)
			HoF << g << "\t" << L(I[0].GetSequence()) << "\t" << mean(P/2,lenghts) << "\t" << mean(P,lenghts) << endl; //Write on file what has happened in this generation
	...
	//Write on file the best of each continent
   	if (rank == 0) {
  		best.open("Best0.dat");		//Write on file the charachteristics of the best individual of the last generation
    	for (int i = 0; i < N; i++) { 
    		best << I[0].GetGene(i) << "\t" << cities[I[0].GetGene(i)][0] << " " << cities[I[0].GetGene(i)][1] << endl;
    	}
    }
    if (rank == 1) {
  		best.open("Best1.dat");		//Write on file the charachteristics of the best individual of the last generation
    	for (int i = 0; i < N; i++) { 
    		best << I[0].GetGene(i) << "\t" << cities[I[0].GetGene(i)][0] << " " << cities[I[0].GetGene(i)][1] << endl;
    	}
    }
    if (rank == 2) {
  		best.open("Best2.dat");		//Write on file the charachteristics of the best individual of the last generation
    	for (int i = 0; i < N; i++) { 
    		best << I[0].GetGene(i) << "\t" << cities[I[0].GetGene(i)][0] << " " << cities[I[0].GetGene(i)][1] << endl;
    	}
    }
    if (rank == 3) {
  		best.open("Best3.dat");		//Write on file the charachteristics of the best individual of the last generation
    	for (int i = 0; i < N; i++) { 
    		best << I[0].GetGene(i) << "\t" << cities[I[0].GetGene(i)][0] << " " << cities[I[0].GetGene(i)][1] << endl;
    	}
    }
...
void Migration_A (int* sent, int* received, int rank) {
	if (rank == 0) {
  		MPI_Send(sent, N, MPI_INTEGER, 1, 1, MPI_COMM_WORLD);
    	MPI_Recv(received, N, MPI_INTEGER, 1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    } else if (rank == 1) { 
    	MPI_Send(sent, N, MPI_INTEGER, 0, 1, MPI_COMM_WORLD);
        MPI_Recv(received, N, MPI_INTEGER, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
   	} else if (rank == 2) {
        MPI_Send(sent, N, MPI_INTEGER, 3, 1, MPI_COMM_WORLD);
        MPI_Recv(received, N, MPI_INTEGER, 3, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
   	} else if (rank == 3) {
      	MPI_Send(sent, N, MPI_INTEGER, 2, 1, MPI_COMM_WORLD);
        MPI_Recv(received, N, MPI_INTEGER, 2, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  	}
  	return;
}

void Migration_B (int* sent, int* received, int rank) {
	if (rank == 0) {
		MPI_Send(sent, N, MPI_INTEGER, 2, 1, MPI_COMM_WORLD);
		MPI_Recv(received, N, MPI_INTEGER, 2, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    } else if (rank == 2) {
...
void Migration_C (int* sent, int* received, int rank) {
	if (rank == 0) {
		MPI_Send(sent, N, MPI_INTEGER, 3, 1, MPI_COMM_WORLD);
		MPI_Recv(received, N, MPI_INTEGER, 3, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    } else if (rank == 3) {
